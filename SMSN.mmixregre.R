##############################################################################################
###########     ALGORITMO EM MULTIVARIADO PARA MISTURAS DE REGRESSAO SMSN              #######
###########     PAPER Luis Heleno Victor                                      ################
##############################################################################################
...
#    g <- length(shape)
  ...
  #    else d <- g*(2*p + length(Sigma[[1]][upper.tri(Sigma[[1]], diag = TRUE)]) ) + (g-1) #mu + shape + Sigma + pi
  #  }
  if(family == "t")      d <- g*length(Sigma[[1]][upper.tri(Sigma[[1]], diag = TRUE)]) + g*nrow(betas) + (g-1) + 1 #sigma + betas + pii + nu
  if(family == "Normal") d <- g*length(Sigma[[1]][upper.tri(Sigma[[1]], diag = TRUE)]) + g*nrow(betas) + (g-1) #sigma + betas + pii + nu
  
  if(family == "Skew.t" || family == "Skew.slash") d <- g*length(Sigma[[1]][upper.tri(Sigma[[1]], diag = TRUE)]) + g*p + g*nrow(betas) + (g-1) + 1 #sigma + shape + betas + pii + nu
  if(family == "Skew.normal") d <- g*length(Sigma[[1]][upper.tri(Sigma[[1]], diag = TRUE)]) + g*p + g*nrow(betas) + (g-1) #sigma + shape + betas + pii + nu
  if(family == "Skew.cn")     d <- g*length(Sigma[[1]][upper.tri(Sigma[[1]], diag = TRUE)]) + g*p + g*nrow(betas) + (g-1) + 1 + 1 #sigma + shape + betas + pii + nu + gamma
  ...
  #icl     <- -2*icl + log(n)*d
  obj.out <- list(beta = betas, Sigma = Sigma, shape = shape, pii = pii, nu = nu, logLik = lk, aic = aic, bic = bic, edc = edc,  iter = count, n = nrow(y), group = cl ,group2 =apply(tal, 1, which.max))
